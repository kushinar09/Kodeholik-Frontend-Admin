...
import { editUser, getUserDetailForAdmin } from "@/lib/api/user_api"
...
const formSchema = z.object({
  ...
  status: z.string().trim().min(1, "Please select a status for this user")
})
...
export default function EditUser({ onNavigate, setCurrentTitleUser }) {
  ...
  const handleEditUser = async (user) => {
    ...
    try {
      ...
      await editUser(apiCall, formPayload, id)
