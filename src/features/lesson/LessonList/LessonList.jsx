...
import { Link, useNavigate, useSearchParams } from "react-router-dom"; // Thêm useNavigate, useSearchParams
...
function LessonList({ onNavigate }) {
  ...
  const [chapterId, setChapterId] = useState(
    searchParams.get("chapterId") || 1
  ); // Lấy chapterId từ URL hoặc mặc định là 1
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      try {
        ...
        const initialChapter = chapterArray.find(
          (chapter) => chapter.id === Number(chapterId)
        );
        setSelectedChapter(
          initialChapter?.title || `Unnamed Chapter (ID: ${chapterId})`
        );
      } catch (error) {
    ...
    fetchChapters();
  }, [chapterId]); // Thêm chapterId vào dependency để cập nhật selectedChapter
  ...
  const handleChapterClick = (chapter) => {
    ...
    setSearchParams({ chapterId: chapter.id }); // Cập nhật URL
  };

  const handleCreateLesson = () => {
    navigate(`/lesson/add?chapterId=${chapterId}`);
  };
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12 text-text-secondary">
                ...
                <p className="text-sm mt-2">
                  Try adjusting your search or chapter selection
                </p>
              </div>
            ) : (
                ...
                <Table className="w-full">
                  <TableHeader className="bg-bg-secondary">
                    <TableRow className="hover:bg-bg-secondary/80">
                      ...
                      <TableHead className="text-text-secondary font-semibold">
                        Action
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem
                                ...
                                onClick={() =>
                                  onNavigate(
                                    `/lesson/${lesson.id}?chapterId=${chapterId}`
                                  )
                                }
                              >
